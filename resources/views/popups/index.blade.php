...
@section('title', 'Popup - Pazar Admin')
...
@section('content')
    ...
    <x-card>
        @if(count($popups) > 0)
            <x-table :headers="['ID', 'Gambar', 'Link', 'Status']">
                @foreach($popups as $popup)
                    <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-4">{{ $popup->pu_id }}</td>
                        <td class="px-6 py-4">
                            @if($popup->pu_image)
                                <img src="{{ asset('storage/' . $popup->pu_image) }}" alt="Popup Image" class="w-16 h-16 object-cover rounded">
                            @else
        ...
        @else
            <div class="py-8 text-center">
                <p class="text-gray-500 dark:text-gray-400">Belum ada popup yang ditambahkan</p>
